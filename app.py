...
# --------------------------------------------------
def generate_recommendations(features, rule_results):

    recommendations = []

    # Use the formal 8-rule engine as the single source of truth.
    for rule in rule_results:
        if not rule["Passed"]:
            recommendations.append(
                f'{rule["Rule ID"]}: {rule["Recommendation"]}'
            )

    # Character-composition guidance: report EVERY missing category.
    if features["uppercase"] == 0:
        recommendations.append(
            "Add uppercase letters (A-Z) to improve character diversity."
        )

    if features["lowercase"] == 0:
        recommendations.append(
            "Add lowercase letters (a-z) to improve character diversity."
        )

    if features["digits"] == 0:
        recommendations.append(
            "Add numeric characters (0-9) to increase character diversity."
        )
    ...
    if not recommendations:
        recommendations.append(
            "No major weakness was detected. Keep the password unique and avoid reusing it across accounts."
        )
